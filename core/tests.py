...
class PedidoTests(TestCase):
    ...
    def test_visualizacao_lista_apenas_do_usuario(self):
        outro_usuario = User.objects.create_user(email='outro@example.com', password='123456')
        Pedido.objects.create(usuario=self.usuario, titulo='Meu Pedido', status='pendente')
        Pedido.objects.create(usuario=outro_usuario, titulo='Outro Pedido', status='pendente')
